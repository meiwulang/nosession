...
public class AutoProjectUtil {
	...
	public static void createService() {
		try {
			...
			// 要生成的根目录
			String daoRoot = getPath("src\\main\\java\\com\\hjh\\mall\\service");
	...
	public static void createDao() {
		try {
			...
			String result = replaceModel(daoTemplate);
			// 要生成的根目录
			String daoRoot = getPath("src\\main\\java\\com\\hjh\\mall\\dao");
			File rootPath = new File(daoRoot);
			// 如果不存在那么久创建
			if (!rootPath.exists())
				rootPath.mkdirs();
	...
	public static void createServiceImpl() {
		try {
			...
			// 要生成的根目录
			String daoRoot = getPath("src\\main\\java\\com\\hjh\\mall\\service\\impl");
			File rootPath = new File(daoRoot);
			// 如果不存在那么久创建
			if (!rootPath.exists())
				rootPath.mkdirs();
	...
	public static void createServiceTest() {
		try {
			...
			String result = replaceModel(serviceTemplate);
			// 要生成的根目录
			String daoRoot = getPath("src\\test\\java\\com\\hjh\\mall\\service\\impl");
			File rootPath = new File(daoRoot);
			// 如果不存在那么久创建
			if (!rootPath.exists())
				rootPath.mkdirs();
	...
	public static void creatSqlmaping() throws Exception {
		Class c = Class.forName("com.hjh.mall.entity." + entity);
		...
		for (int i = 0; i < f.length; i++) {
			...
			if (!entityid.equals(f[i].getName())) {// 如果是id 则不能赋值
	...
	private static String author = "陈士俊";// 作者
	private static String description = "类目";// 中文提示
	private static String date = new SimpleDateFormat("yyyy年MM月dd日").format(new Date());// 时间格式
	private static String entity = "Category";// 实体名
	// 【Entity】
	private static String lowEntity = "category";// 实体对象【entity】
	private static String entityid = "category_id";// 数据库表主键【entity_id】
	private static String tablename = table_name;// 表名【entity】
