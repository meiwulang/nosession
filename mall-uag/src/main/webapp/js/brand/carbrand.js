...
$("#export").on('click',function(){
	...
	if(false){
		...
		var form = $("<form style='display:none' target='' method='post' action='/carbrand/export'><input type='hidden' name='page' value='"+
...
function listsReady(data){
		$.ajax({
			contentType: "application/json; charset=utf-8",
			type: 'POST',
			url:"/carbrand/queryblur",
			dataType: 'json',
			data: JSON.stringify(data),
			success:function(responseText, textStatus, jqXHR){
				var dataHtml = '<tr><th><input id="checkAll" type="checkbox" name="checkbox1" value="checkbox"></th><th>代码</th><th>品牌</th><th>logo</th><th>机型数</th><th>排序</th><th>创建日期</th><th>创建者</th><th>最后操作日期</th><th>最后操作者</th><th>状态</th><th>操作</th></tr>';
				if(responseText.error_no == '0'){
					...
					resultList.forEach(function(item) {
						...
						dataHtml += '<tr class="tableEdit"><td><input id="" class="cklist" type="checkbox" name="checkbox1" value="'+item.brand_id+'"></td>'
...
$('.mainLists').on('click','.allowUn',function(){
	...
	if(r == true){
		...
		$.ajax({
			contentType: "application/json; charset=utf-8",
			type: 'POST',
			url:"/carbrand/brand_update_status",
			dataType: 'json',
